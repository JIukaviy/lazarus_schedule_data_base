...
procedure TAnimatedGrid.DelAnimations;
var
  i, j: Integer;
begin
  j := 0;
...
procedure TAnimatedGrid.Animation(Sender: TObject);
var
  i: Integer;
  dheight: Integer;
  del: boolean;
...
procedure TAnimatedGrid.AddAnimation(aRow, aTargetHeight: Integer);
var
  i: Integer;
begin
  if not FAnimate then begin
    RowHeights[aRow] := aTargetHeight;
    Exit;
  end;
  for i := 0 to High(FTargetHeights) do
...
constructor TAnimatedGrid.Create(aOwner: TComponent);
begin
...
procedure TAnimatedGrid.CloseRow(aRow: Integer);
begin
  AddAnimation(aRow, DefaultRowHeight);
end;

procedure TAnimatedGrid.SetRowHeight(aRow, aHeight: Integer);
begin
